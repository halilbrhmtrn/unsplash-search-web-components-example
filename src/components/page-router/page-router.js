...
export default class PageRouter extends HTMLElement {
    constructor() {
        ...
        this.getCurrentPage = () => _currentPage
        this.getDefaultPage = () => _defaultPage
    }
    ...
    connectedCallback() {
        ...
        window.onerror = (message, file, line) => {
            console.log(
                `${
                    'Unsplash Search Client :' + 'An error occurred at line '
        ...
        window.onclick = (event) => {
            if (
                ...
                if (parentRoot) {
                    const modalElement =
                        parentRoot.querySelector('detail-modal')
                    if (
                        modalElement &&
...
function renderCorrectPage() {
    ...
    const { state } = this.getCurrentPage()
    if (state) {
    ...
    const { title } = this.getCurrentPage()
