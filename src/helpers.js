...
const attachCallbacks = (html, element) => {
  ...
  let matches = returnedHTML.match(regex2);
  if (matches) {
    let evaledMatches = matches.map((match) => {
      return eval(match);
    });
    ...
    [].forEach.call(listItems, (element) => {
      element.addEventListener("click", () => {
        ...
        if (find(`.${prefix}-dropdown-container .selected`))
          find(`.${prefix}-dropdown-container .selected`).classList.remove(
            "selected"
          );
        ...
        selectedItemOnLabel.textContent = element.getAttribute("value");
        selectedItemOnLabel.classList.add("selected");
        element.classList.add("selected");
        elmHided = element;
        element.style.display = "none";
        dropdownContainerList.style.display = "none";
        selectedItemOnLabel.textContent = element.textContent;
